...
import { UserT, db, eq } from "astro:db";
...
export const DELETE: APIRoute = async ({ params }) => {
  ...
  try {
    const res = await db.delete(UserT).where(eq(UserT.id, id));
    if (res) {
...
export const PATCH: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    const user = await db.select().from(UserT).where(eq(UserT.id, id));
    if (!user) {
    ...
    const res = await db.update(UserT).set({
      name,
      lastName,
...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    const res = await db.select().from(UserT).where(eq(UserT.id, id));
